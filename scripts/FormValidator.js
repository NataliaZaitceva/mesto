...
export class FormValidator{
    constructor (settings, formPopup){
     ...
     this._inputList = settings.formList;
        ...
        _showInputError = (inputsEdit, errorMessage) => {//показать инпут с ошибкой
         
          const formError = this._formElement.querySelector(`#${inputsEdit.id}-error`);
          inputsEdit.classList.add(this._inputErrorClass);
            formError.textContent = errorMessage;
            formError.classList.add(this._errorClass);
        }
        _hideInputError = (inputsEdit) => { //скрыть инпут с ошибкой, если ее нет
            const formError = this._formElement.querySelector(`#${inputsEdit.id}-error`);
            inputsEdit.classList.remove(this._inputErrorClass);
            formError.classList.remove(this._errorClass);
            formError.textContent = '';
        }
       _checkInputValidity = (inputsEdit) => {  //сообщения об ошибке
        ...
        _hasInvalidInput(inputList) { //валидность импута
            return inputList.some(inputsEdit => {
                return !inputsEdit.validity.valid;
            });
        }
        _switchButton(inputList, submitButton) { //деактивированный вид кнопки
...
enableValidation (){ //валидация формы
